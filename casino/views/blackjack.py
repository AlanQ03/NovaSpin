...
#             bet = int(request.POST.get('bet'))
#         except (ValueError, TypeError):
#             return render(request, 'pages/blackjack.html', {'error': 'Invalid bet amount'})

#         if bet <= 0 or bet > user.balance:
#             return render(request, 'pages/blackjack.html', {'error': 'Insufficient balance'})
...
#         action = request.POST.get('action')
#         game = Blackjack()
#         game.player_hand = request.session.get('player_hand', [])
#         game.dealer_hand = request.session.get('dealer_hand', [])
